...
const SUPABASE_URL = "https://uaimnefcmjanwwmptrzf.supabase.co";   // ex: "https://xyzxyz.supabase.co"
const SUPABASE_KEY = "sb_publishable_1IgH7FWZvr8pOsEaj-ilUA_aXqf_Wgz";   // ex: "eyJhbGci..."

const sb = (() => {
  if (!SUPABASE_URL || !SUPABASE_KEY) return null;
  try { return window.supabase?.createClient(SUPABASE_URL, SUPABASE_KEY); } catch { return null; }
})();

// Auth state — mis à jour dès que la session change
let sbUser = null;
if (sb) sb.auth.onAuthStateChange((_, s) => { sbUser = s?.user ?? null; });
const getSbUser = () => sbUser;

// ── CLOUD SYNC (fire-and-forget, n'interrompt jamais l'UI) ────────────────────
async function cloudPush(table, payload) {
  if (!sb || !getSbUser()) return;
  try {
    await sb.from(table).upsert({ ...payload, user_id: getSbUser().id, updated_at: new Date().toISOString() });
  } catch(e) { console.warn("[GAINZ cloud push]", e?.message); }
}

async function cloudPull() {
  if (!sb || !getSbUser()) return null;
  try {
    const uid = getSbUser().id;
    const [progs, hist, perf, prof] = await Promise.all([
      sb.from("gainz_programs").select("data").eq("user_id", uid),
      sb.from("gainz_history").select("data").eq("user_id", uid).order("ts", { ascending:false }).limit(60),
      sb.from("gainz_perf").select("data").eq("user_id", uid).maybeSingle(),
      sb.from("gainz_profile").select("data").eq("user_id", uid).maybeSingle(),
    ]);
    return {
      programs: progs.data?.map(r => r.data) ?? [],
      history:  hist.data?.map(r => r.data) ?? [],
      perf:     perf.data?.data ?? {},
      profile:  prof.data?.data ?? null,
    };
  } catch(e) { console.warn("[GAINZ cloud pull]", e?.message); return null; }
}
...
const TRANSLATIONS = {
  fr: {
    appName: "GAINZ",
    // Nav
    navCreate: "Créer", navPrograms: "Programmes", navNutrition: "Nutrition", navProfile: "Profil", navProgress: "Progrès",
    // Create
  ...
  en: {
    appName: "GAINZ",
    navCreate: "Create", navPrograms: "Programs", navNutrition: "Nutrition", navProfile: "Profile", navProgress: "Progress",
...
async function savePrograms(list) {
  ...
  for (const p of list) cloudPush("gainz_programs", { id: p.id, data: p });
}
...
async function saveProfile(p) {
  try { await window.storage.set("gainz:profile", JSON.stringify(p)); } catch {}
  if (getSbUser?.()) cloudPush("gainz_profile", { id: getSbUser().id, data: p });
}
...
async function saveHistory(h) {
  try { await window.storage.set("gainz:history", JSON.stringify(h)); } catch {}
  for (const e of h.slice(0, 5)) cloudPush("gainz_history", { id: e.id, data: e, ts: e.ts });
}
async function loadPerf() { try { const r = await window.storage.get("gainz:perf"); return r ? JSON.parse(r.value) : {}; } catch { return {}; } }
async function savePerf(p) {
  try { await window.storage.set("gainz:perf", JSON.stringify(p)); } catch {}
  if (getSbUser?.()) cloudPush("gainz_perf", { id: getSbUser().id, data: p });
}
...
export default function App() {
  ...
  const [sbUserState, setSbUserState] = useState(null);
  ...
  const pullAndMerge = async () => {
    ...
    // Get current local state at time of merge
    const localPrograms = await loadPrograms();
    const localHistory  = await loadHistory();
    const localPerf     = await loadPerf();
    const localProfile  = await loadProfile();

    // Pull cloud → overwrite local if cloud has data
    if (cloud?.programs?.length)          { setSavedPrograms(cloud.programs); await window.storage.set("gainz:programs", JSON.stringify(cloud.programs)); }
    if (cloud?.history?.length)           { setHistory(cloud.history);        await window.storage.set("gainz:history",  JSON.stringify(cloud.history)); }
    if (cloud?.perf && Object.keys(cloud.perf).length) { setPerf(cloud.perf); await window.storage.set("gainz:perf",     JSON.stringify(cloud.perf)); }
    if (cloud?.profile)                   { setProfile(cloud.profile);        await window.storage.set("gainz:profile",  JSON.stringify(cloud.profile)); }

    // Push local data to cloud for anything cloud didn't have
    if (!cloud?.programs?.length && localPrograms.length)
      for (const p of localPrograms) cloudPush("gainz_programs", { id: p.id, data: p });
    if (!cloud?.history?.length && localHistory.length)
      for (const e of localHistory.slice(0, 20)) cloudPush("gainz_history", { id: e.id, data: e, ts: e.ts });
    if ((!cloud?.perf || !Object.keys(cloud.perf).length) && Object.keys(localPerf).length)
      cloudPush("gainz_perf", { id: getSbUser().id, data: localPerf });
    if (!cloud?.profile && localProfile)
      cloudPush("gainz_profile", { id: getSbUser().id, data: localProfile });
  ...
  // Log a completed set: { exName, weight (kg or 0 for BW), reps, date }
  const handleLogSet = async (exName, weight, reps) => {
    const key = exName.toLowerCase().replace(/[^a-z0-9]/g, "_").slice(0, 40);
    const entry = { w: parseFloat(weight) || 0, r: parseInt(reps) || 0, d: new Date().toISOString().slice(0, 10), ts: Date.now() };
    const updated = { ...perf, [key]: [...(perf[key] || []), entry].slice(-60) };
    setPerf(updated);
    await savePerf(updated);
  };
  ...
  if (dailyView) {
    return (
      <LangContext.Provider value={lang}>
        <div className="app">
          <DailyView program={dailyView} onBack={() => setDailyView(null)} onWorkoutDone={handleWorkoutDone} onLogSet={handleLogSet} perf={perf} lang={lang} />
        </div>
  ...
  return (
    <LangContext.Provider value={lang}>
      <div className="app">
        ...
        {tab === "programs"  && <ProgramsView programs={savedPrograms} onOpen={setDailyView} onDelete={handleDelete} lang={lang} isPremium={isPremium} onShowPremium={() => setShowPremiumModal(true)} onShare={handleShare} onGoCreate={() => setTab("create")} sbUser={sbUserState} onSynced={pullAndMerge} />}
        {tab === "progress"  && <ProgressView perf={perf} history={history} lang={lang} profile={profile} onGoCreate={() => setTab("create")} />}
        {tab === "nutrition" && <NutritionView profile={profile} onGoToProfile={() => setTab("profile")} lang={lang} />}
        {tab === "profile"   && <ProfileView profile={profile} history={history} onSave={handleProfileSave} lang={lang} onLangChange={switchLang} currentLang={lang} theme={theme} onThemeChange={switchTheme} sbUser={sbUserState} cloudSyncing={cloudSyncing} />}
...
// ── CLOUD LOGIN SHORTCUT (shown on empty screen on new device) ────────────────
function CloudLoginShortcut({ lang, onSynced }) {
  ...
  const handle = async () => {
    ...
    setLoading(false);
    if (error) {
      setMsg(lang === "en" ? "Wrong email or password" : "Email ou mot de passe incorrect");
    } else {
      setMsg(lang === "en" ? "✓ Signed in — syncing your data…" : "✓ Connecté — récupération de tes données…");
  ...
  return (
    <div style={{ display:"flex", flexDirection:"column", gap:8 }}>
      <input
        ...
        className="input-field" style={{ fontSize:14, textAlign:"center" }}
      />
      <input
        type="password" placeholder={lang === "en" ? "Password" : "Mot de passe"}
        value={password} onChange={e => setPassword(e.target.value)}
        className="input-field" style={{ fontSize:14, textAlign:"center" }}
      />
      <button
        onClick={handle} disabled={loading}
        style={{ width:"100%", background:"var(--surface)", border:"1px solid var(--gold)", borderRadius:12, padding:"12px", fontSize:13, fontWeight:700, color:"var(--gold)", cursor:"pointer", fontFamily:"Outfit", opacity: loading ? 0.7 : 1 }}>
        {loading ? "…" : (lang === "en" ? "☁️ Sign in & sync" : "☁️ Se connecter & synchroniser")}
      </button>
      {msg && <div style={{ fontSize:12, color: msg.startsWith("✓") ? "var(--green)" : "var(--red)", textAlign:"center", lineHeight:1.5 }}>{msg}</div>}
    </div>
  );
}

// ── PROGRAMS VIEW ─────────────────────────────────────────────────────────────
function ProgramsView({ programs, onOpen, onDelete, lang, onShare, onGoCreate, sbUser, onSynced }) {
  const t = (key) => TRANSLATIONS[lang]?.[key] ?? TRANSLATIONS.fr[key] ?? key;
  return (
    <>
      <div className="app-header">
      ...
      <div className="page-content">
        {programs.length === 0 ? (
          <div className="empty-state fade-up">
            ...
            {/* Cloud login shortcut — visible on new device */}
            {sb && !sbUser && (
              <div style={{ marginTop:20, width:"100%", maxWidth:260 }}>
                <div style={{ display:"flex", alignItems:"center", gap:8, marginBottom:12 }}>
                  <div style={{ flex:1, height:1, background:"var(--border)" }}/>
                  <div style={{ fontSize:11, color:"var(--text3)", fontFamily:"Outfit" }}>{lang === "en" ? "already have an account?" : "déjà un compte ?"}</div>
                  <div style={{ flex:1, height:1, background:"var(--border)" }}/>
                </div>
                <CloudLoginShortcut lang={lang} onSynced={onSynced} />
              </div>
            )}
            {sb && sbUser && (
              <div style={{ marginTop:16, fontSize:12, color:"var(--green)", textAlign:"center" }}>
                ✓ {lang === "en" ? "Synced — pull your data in Settings" : "Synchronisé — récupère tes données dans Réglages"}
              </div>
            )}
          </div>
        ) : (
...
// ── DAILY VIEW ────────────────────────────────────────────────────────────────
function DailyView({ program, onBack, onWorkoutDone, onLogSet, perf = {}, lang = "fr" }) {
  const t = (key) => TRANSLATIONS[lang]?.[key] ?? TRANSLATIONS.fr[key] ?? key;
  const [dayIdx, setDayIdx] = useState(0);
  // setsCompleted: { "dayIdx-exIdx": number } — how many sets done for each exercise
  const [setsCompleted, setSetsCompleted] = useState({});
  const [logInput, setLogInput] = useState({}); // { "dayIdx-exIdx": { weight, reps, open } }
  ...
  // An exercise is "done" when all its sets are completed
  const getExSets = (ex) => parseInt(ex.sets) || 3;
  const isExDone = (j) => (setsCompleted[`${dayIdx}-${j}`] || 0) >= getExSets(day?.exercises?.[j]);
  const doneEx = day?.exercises?.filter((_, j) => isExDone(j)).length || 0;
  ...
  // Open log panel for next set
  const openSetLog = (j, ex) => {
    const k = `${dayIdx}-${j}`;
    const defaultReps = ex.reps?.split(/[–-]/)?.[1]?.trim() || ex.reps?.split(/[–-]/)?.[0]?.trim() || "10";
    setLogInput(p => ({ ...p, [k]: { weight: p[k]?.weight || "", reps: defaultReps, open: true } }));
  };

  // Undo last set
  const undoSet = (j) => {
    const k = `${dayIdx}-${j}`;
    const current = setsCompleted[k] || 0;
    if (current <= 0) return;
    setSetsCompleted(p => ({ ...p, [k]: current - 1 }));
    setLogInput(p => ({ ...p, [k]: { ...p[k], open: false } }));
  };

  // Confirm a set: log it, increment counter, start rest timer
  const confirmSet = (j, ex) => {
    const k = `${dayIdx}-${j}`;
    const inp = logInput[k] || {};
    const totalSets = getExSets(ex);
    const newCount = (setsCompleted[k] || 0) + 1;

    setSetsCompleted(p => ({ ...p, [k]: newCount }));
    setLogInput(p => ({ ...p, [k]: { ...p[k], open: false } }));

    if (onLogSet) onLogSet(ex.name, inp.weight || 0, inp.reps || 10);

    // Start rest timer after every set (not just the last one)
    if (ex.restSec) {
      const isLastSet = newCount >= totalSets;
      const nextName = isLastSet
        ? day?.exercises?.[j + 1]?.name   // next exercise
        : `${ex.name} — ${lang === "en" ? "set" : "série"} ${newCount + 1}/${totalSets}`; // next set of same exercise
      startTimer(ex.restSec, nextName);
    }
  };
        ...
        {day?.isRest ? (
          ...
          <div className="fade-up">
            ...
            {day?.exercises?.map((ex, j) => {
              const k = `${dayIdx}-${j}`;
              const totalSets = getExSets(ex);
              const doneSets = setsCompleted[k] || 0;
              const done = doneSets >= totalSets;
              const logOpen = logInput[k]?.open;
              return (
                <div key={j} className={`ex-card ${done ? "completed" : ""} ${ex.isFinisher ? "finisher-card" : ""}`}>
                  <div className="ex-card-top">
                    ...
                    <div className="ex-card-info">
                      <div style={{ display: "flex", alignItems: "center", gap: 6, flexWrap: "wrap" }}>
                        ...
                        {/* PR badge */}
                        {(() => {
                          const key = ex.name.toLowerCase().replace(/[^a-z0-9]/g, "_").slice(0, 40);
                          const logs = perf[key] || [];
                          if (logs.length === 0) return null;
                          const best = Math.max(...logs.map(l => l.w || 0));
                          if (best === 0) return null;
                          return <div style={{ background:"rgba(201,168,76,0.12)", border:"1px solid rgba(201,168,76,0.3)", borderRadius:5, padding:"1px 8px", fontSize:9, fontWeight:700, color:"var(--gold)", fontFamily:"Outfit", letterSpacing:0.3 }}>PR {best}kg</div>;
                        })()}
                      </div>
                  ...
                  {/* Per-set progress dots */}
                  <div style={{ display:"flex", alignItems:"center", gap:6, marginBottom:10 }}>
                    {Array.from({ length: totalSets }, (_, i) => (
                      <div key={i} style={{
                        width: i < doneSets ? 28 : 24,
                        height: i < doneSets ? 28 : 24,
                        borderRadius: "50%",
                        background: i < doneSets ? "var(--green)" : "var(--surface2)",
                        border: `2px solid ${i < doneSets ? "var(--green)" : i === doneSets ? "var(--gold)" : "var(--border)"}`,
                        display:"flex", alignItems:"center", justifyContent:"center",
                        fontSize: 10, fontWeight:700, fontFamily:"Outfit",
                        color: i < doneSets ? "#0C0C0E" : i === doneSets ? "var(--gold)" : "var(--text3)",
                        transition:"all 0.2s", flexShrink:0,
                      }}>
                        {i < doneSets ? "✓" : i + 1}
                      </div>
                    ))}
                    <div style={{ fontSize:11, color: done ? "var(--green)" : "var(--text3)", fontFamily:"Outfit", fontWeight:600, marginLeft:4 }}>
                      {done
                        ? (lang === "en" ? "All sets done ✓" : "Toutes les séries ✓")
                        : `${lang === "en" ? "Set" : "Série"} ${doneSets + 1} / ${totalSets}`}
                    </div>
                  </div>

                  {/* Log input panel */}
                  {logOpen && (
                    <div style={{ background:"var(--surface2)", border:"1px solid var(--border)", borderRadius:12, padding:"12px 13px", marginBottom:10 }}>
                      <div style={{ fontSize:11, fontWeight:700, letterSpacing:1, textTransform:"uppercase", color:"var(--text3)", marginBottom:9, fontFamily:"Outfit" }}>
                        {lang === "en" ? `Log set ${doneSets + 1}/${totalSets}` : `Série ${doneSets + 1}/${totalSets}`}
                      </div>
                      <div style={{ display:"flex", gap:8, marginBottom:10 }}>
                        <div style={{ flex:1 }}>
                          <div style={{ fontSize:10, color:"var(--text3)", marginBottom:4, fontFamily:"Outfit" }}>{lang === "en" ? "Weight (kg)" : "Charge (kg)"}</div>
                          <input
                            type="number" inputMode="decimal" placeholder={lang === "en" ? "0 = bodyweight" : "0 = poids du corps"}
                            value={logInput[k]?.weight || ""}
                            onChange={e => setLogInput(p => ({ ...p, [k]: { ...p[k], weight: e.target.value } }))}
                            style={{ width:"100%", background:"var(--surface)", border:"1px solid var(--border)", borderRadius:9, padding:"9px 11px", fontSize:15, color:"var(--text)", fontFamily:"Outfit", outline:"none" }}
                          />
                        </div>
                        <div style={{ flex:1 }}>
                          <div style={{ fontSize:10, color:"var(--text3)", marginBottom:4, fontFamily:"Outfit" }}>{lang === "en" ? "Reps done" : "Reps effectuées"}</div>
                          <input
                            type="number" inputMode="numeric"
                            value={logInput[k]?.reps || ""}
                            onChange={e => setLogInput(p => ({ ...p, [k]: { ...p[k], reps: e.target.value } }))}
                            style={{ width:"100%", background:"var(--surface)", border:"1px solid var(--border)", borderRadius:9, padding:"9px 11px", fontSize:15, color:"var(--text)", fontFamily:"Outfit", outline:"none" }}
                          />
                        </div>
                      </div>
                      <div style={{ display:"flex", gap:7 }}>
                        <button
                          onClick={() => setLogInput(p => ({ ...p, [k]: { ...p[k], open: false } }))}
                          style={{ flex:1, background:"var(--surface)", color:"var(--text3)", border:"1px solid var(--border)", borderRadius:10, padding:"10px", fontSize:12, fontWeight:600, fontFamily:"Outfit", cursor:"pointer" }}>
                          {lang === "en" ? "Cancel" : "Annuler"}
                        </button>
                        <button
                          onClick={() => confirmSet(j, ex)}
                          style={{ flex:2, background:"var(--gold)", color:"#0C0C0E", border:"none", borderRadius:10, padding:"10px", fontSize:13, fontWeight:700, fontFamily:"Outfit", cursor:"pointer" }}>
                          ✓ {lang === "en" ? "Done — start rest" : "Fait — lancer le repos"}
                        </button>
                      </div>
                    </div>
                  )}
                  ...
                  {/* Action buttons */}
                  {!done && !logOpen && (
                    <button className="ex-done-btn" onClick={() => openSetLog(j, ex)}>
                      {doneSets === 0
                        ? (lang === "en" ? `Start set 1/${totalSets}` : `Démarrer série 1/${totalSets}`)
                        : (lang === "en" ? `Set ${doneSets + 1}/${totalSets} — ready` : `Série ${doneSets + 1}/${totalSets} — prêt`)}
                    </button>
                  )}
                  {done && (
                    <button className="ex-done-btn" onClick={() => undoSet(j)}>
                      {t("markUndo")}
                    </button>
                  )}
                </div>
...
function generateShareCard(canvasRef, data, theme) {
  const canvas = canvasRef.current;
  if (!canvas) return;
  const ctx = canvas.getContext("2d");
  // 9:16 portrait — perfect for Stories
  const W = 1080, H = 1920;
  ...
  // ── BACKGROUND — always dark, more dramatic ──────────────────────────────
  ctx.fillStyle = "#0C0C0E";
  ctx.fillRect(0, 0, W, H);

  // Radial glow behind the big number
  const grd = ctx.createRadialGradient(W/2, H*0.42, 0, W/2, H*0.42, 500);
  grd.addColorStop(0, "rgba(201,168,76,0.18)");
  ...
  // Gold G badge
  ctx.fillStyle = gold;
  roundRect(ctx, 80, 100, 80, 80, 20); ctx.fill();
  ctx.fillStyle = "#0C0C0E";
  ctx.font = "bold 48px sans-serif";
  ctx.textAlign = "center";
  ctx.fillText("G", 120, 157);

  ctx.fillStyle = "#F5F5F7";
  ctx.font = "bold 44px sans-serif";
  ctx.textAlign = "left";
  ctx.fillText("GAINZ", 182, 157);
  ...
  ctx.fillText(data.label.toUpperCase(), W/2, centerY - 40);

  // THE NUMBER
  ctx.fillStyle = "#F5F5F7";
  ctx.font = `bold ${data.bigNum.toString().length > 3 ? "260" : "320"}px sans-serif`;
  ctx.textAlign = "center";
  ctx.fillText(data.bigNum, W/2, centerY + 220);

  // Unit below number
  ctx.fillStyle = gold2;
  ctx.font = "bold 52px sans-serif";
  ctx.fillText(data.unit, W/2, centerY + 310);
  ...
  // ── SECONDARY STATS (2 pills side by side) ────────────────────────────────
  const statY = H * 0.72;
  data.secondaryStats.forEach((s, i) => {
    const x = i === 0 ? W * 0.27 : W * 0.73;
    // Pill background
    ctx.fillStyle = "rgba(201,168,76,0.12)";
    roundRect(ctx, x - 160, statY - 50, 320, 120, 20); ctx.fill();
    ctx.strokeStyle = "rgba(201,168,76,0.25)";
    ctx.lineWidth = 1.5;
    roundRect(ctx, x - 160, statY - 50, 320, 120, 20); ctx.stroke();

    ctx.fillStyle = gold2;
    ctx.font = "bold 64px sans-serif";
    ctx.textAlign = "center";
    ctx.fillText(s.val, x, statY + 28);
    ctx.fillStyle = "rgba(255,255,255,0.4)";
    ctx.font = "24px sans-serif";
    ctx.fillText(s.label, x, statY + 62);
  });

  // ── TOP PR (if exists) ────────────────────────────────────────────────────
  if (data.topPR) {
    const prY = H * 0.84;
    ctx.fillStyle = "rgba(245,223,160,0.07)";
    roundRect(ctx, 80, prY - 50, W - 160, 110, 22); ctx.fill();
    ctx.strokeStyle = gold;
    ctx.lineWidth = 1.5;
    roundRect(ctx, 80, prY - 50, W - 160, 110, 22); ctx.stroke();

    ctx.fillStyle = gold;
    ctx.font = "bold 26px sans-serif";
    ctx.textAlign = "left";
    ctx.fillText("🏆  RECORD PERSONNEL", 120, prY - 5);
    ctx.fillStyle = "#F5F5F7";
    ctx.font = "bold 40px sans-serif";
    ctx.fillText(data.topPR.name, 120, prY + 44);
    ctx.fillStyle = gold2;
    ctx.font = "bold 40px sans-serif";
    ctx.textAlign = "right";
    ctx.fillText(`${data.topPR.pr} kg`, W - 120, prY + 44);
  }
  ...
  ctx.textAlign = "center";
  ctx.fillText("gainz.app", W/2, H - 80);
...
// ── PROGRESS VIEW ─────────────────────────────────────────────────────────────
function ProgressView({ perf, history, lang, profile, onGoCreate }) {
  ...
  // Build exercise list
  const exercises = Object.entries(perf)
    .map(([key, logs]) => ({
      key,
      name: key.replace(/_/g, " ").replace(/\b\w/g, c => c.toUpperCase()),
      logs: [...logs].sort((a, b) => a.ts - b.ts),
      pr: Math.max(...logs.map(l => l.w || 0)),
      lastW: logs[logs.length - 1]?.w || 0,
      count: logs.length,
    }))
    .sort((a, b) => b.count - a.count);

  const totalSessions = history.length;
  const thisWeek = history.filter(h => (Date.now() - h.ts) < 7 * 86400000).length;

  // ── WEEKLY CHALLENGE ──────────────────────────────────────────────────────
  const weeklyChallenge = React.useMemo(() => {
    if (exercises.length === 0) return null;
    // Pick the exercise with most data for a meaningful challenge
    const top = exercises.filter(e => e.pr > 0)[0] || exercises[0];
    if (!top) return null;

    const weekAgo = Date.now() - 7 * 86400000;
    const recentLogs = top.logs.filter(l => l.ts > weekAgo);
    const recentBest = recentLogs.length ? Math.max(...recentLogs.map(l => l.w || 0)) : 0;
    const target = top.pr > 0 ? Math.round(top.pr * 1.025 * 2) / 2 : null; // +2.5%, rounded to 0.5kg
    const progress = recentBest > 0 ? Math.min(100, Math.round((recentBest / (target || top.pr)) * 100)) : 0;
    const done = recentBest >= (target || top.pr);

    return { name: top.name, current: top.pr, target, progress, done, recentBest };
  }, [exercises]);
  ...
  const openShare = () => {
    setShowShare(true);
    setTimeout(() => {
      const topPR = exercises.filter(e => e.pr > 0).sort((a,b) => b.pr - a.pr)[0];
      const data = {
        bigNum: totalSessions,
        label: lang === "en" ? "total sessions" : "séances au total",
        unit:  lang === "en" ? "workouts completed" : "entraînements complétés",
        secondaryStats: [
          { val: thisWeek,           label: lang === "en" ? "this week" : "cette sem." },
          { val: exercises.filter(e => e.pr > 0).length, label: "records" },
        ],
        topPR: topPR ? { name: topPR.name, pr: topPR.pr } : null,
      };
      generateShareCard(canvasRef, data, theme);
    }, 80);
  };
  ...
  const shareCard = async () => {
    ...
    try {
      canvas.toBlob(async (blob) => {
        const file = new File([blob], "gainz-progress.png", { type: "image/png" });
        if (navigator.share && navigator.canShare({ files: [file] })) {
          await navigator.share({ files: [file], title: "GAINZ — Ma progression" });
        } else {
          downloadCard();
        }
      });
  ...
  const SparkLine = ({ logs }) => {
    if (logs.length < 2) return (
      <div style={{ fontSize:11, color:"var(--text3)", padding:"14px 0", textAlign:"center" }}>
        {lang === "en" ? "Log more sets to see your curve" : "Enregistre d'autres séries pour voir ta courbe"}
      </div>
    );
    const vals = logs.map(l => l.w || 0);
    ...
    return (
      <svg viewBox={`0 0 ${W} ${H}`} style={{ width:"100%", height:H }}>
        ...
        {points.map(([x,y], i) => (
          <circle key={i} cx={x} cy={y} r={i === points.length-1 ? 5 : 3} fill="var(--gold)" opacity={i===points.length-1?1:0.4}/>
        ))}
        <text x={pad} y={12} fontSize="9" fill="var(--text3)" fontFamily="Outfit">{maxV}kg</text>
        <text x={pad} y={H-2} fontSize="9" fill="var(--text3)" fontFamily="Outfit">{minV}kg</text>
        <text x={W-pad} y={H-2} fontSize="9" fill="var(--text3)" fontFamily="Outfit" textAnchor="end">
          {logs[logs.length-1]?.d}
        </text>
      </svg>
    );
  };

  const selEx = selected ? exercises.find(e => e.key === selected) : null;

  return (
    <>
      <div className="app-header">
        <div className="logo-mark"><div className="logo-icon">G</div><span className="logo-text">GAINZ</span></div>
        <div className="header-badge">{lang === "en" ? "Progress" : "Progrès"}</div>
      </div>
      ...
      {showShare && (
        <div
          ...
          style={{ position:"fixed", inset:0, background:"rgba(0,0,0,0.9)", zIndex:200, display:"flex", flexDirection:"column", alignItems:"center", justifyContent:"flex-start", padding:"16px 16px 32px", overflowY:"auto" }}>
          <div
            onClick={e => e.stopPropagation()}
            style={{ background:"var(--surface)", borderRadius:20, padding:16, width:"100%", maxWidth:380, marginTop:"auto", marginBottom:"auto" }}>
            {/* Header */}
            <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:12 }}>
              <div style={{ fontFamily:"Outfit", fontSize:15, fontWeight:700, color:"var(--text)" }}>
                {lang === "en" ? "Share my progress" : "Partager ma progression"}
              </div>
              <button onClick={() => setShowShare(false)} style={{ width:32, height:32, background:"var(--surface2)", border:"1px solid var(--border)", borderRadius:8, fontSize:14, color:"var(--text3)", cursor:"pointer", display:"flex", alignItems:"center", justifyContent:"center", flexShrink:0 }}>✕</button>
            </div>
            {/* Canvas preview — capped height so buttons always visible */}
            <div style={{ position:"relative", width:"100%", marginBottom:12 }}>
              <canvas ref={canvasRef} style={{ width:"100%", aspectRatio:"9/16", borderRadius:12, display:"block", background:"#0C0C0E", maxHeight:"55vh", objectFit:"contain" }} />
            </div>
            {/* Info */}
            <div style={{ fontSize:11, color:"var(--text3)", textAlign:"center", marginBottom:10 }}>
              {lang === "en" ? "Full resolution 1080×1920 — perfect for Stories" : "Pleine résolution 1080×1920 — parfait pour les Stories"}
            </div>
            {/* Action buttons — always visible */}
            <div style={{ display:"flex", gap:8 }}>
              <button onClick={downloadCard} style={{ flex:1, background:"var(--surface2)", border:"1px solid var(--border)", borderRadius:12, padding:"13px 8px", fontSize:13, fontWeight:600, color:"var(--text)", cursor:"pointer", fontFamily:"Outfit" }}>
                ↓ {lang === "en" ? "Save" : "Enregistrer"}
              </button>
              <button onClick={shareCard} style={{ flex:2, background:"var(--gold)", border:"none", borderRadius:12, padding:"13px 8px", fontSize:13, fontWeight:700, color:"#0C0C0E", cursor:"pointer", fontFamily:"Outfit" }}>
                ↗ {lang === "en" ? "Share" : "Partager"}
              </button>
            </div>
          </div>
      ...
      <div className="page-content fade-up">
        ...
        <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr 1fr", gap:8, marginBottom:14 }}>
          {[
            { val: totalSessions, label: lang === "en" ? "Sessions" : "Séances" },
            { val: thisWeek,      label: lang === "en" ? "This week" : "Cette sem." },
            { val: exercises.length, label: lang === "en" ? "Exercises" : "Exercices" },
          ].map((s, i) => (
            <div key={i} style={{ background:"var(--surface)", border:"1px solid var(--border)", borderRadius:14, padding:"13px 10px", textAlign:"center" }}>
              <div style={{ fontFamily:"Outfit", fontSize:22, fontWeight:800, color:"var(--gold)", lineHeight:1 }}>{s.val}</div>
              <div style={{ fontSize:10, color:"var(--text3)", marginTop:4, fontFamily:"Outfit", fontWeight:600 }}>{s.label}</div>
            </div>
          ))}
        </div>
        ...
        {/* Share card button */}
        {exercises.length > 0 && (
          <button onClick={openShare} style={{ width:"100%", background:"var(--surface)", border:"1px solid var(--border)", borderRadius:14, padding:"12px 16px", display:"flex", alignItems:"center", justifyContent:"space-between", cursor:"pointer", marginBottom:16, WebkitTapHighlightColor:"transparent" }}>
            <div style={{ display:"flex", alignItems:"center", gap:10 }}>
              <div style={{ width:32, height:32, background:"var(--gold-dim)", borderRadius:9, display:"flex", alignItems:"center", justifyContent:"center", fontSize:16 }}>↗</div>
              <div style={{ textAlign:"left" }}>
                <div style={{ fontFamily:"Outfit", fontSize:13, fontWeight:700, color:"var(--text)" }}>{lang === "en" ? "Share my progress" : "Partager ma progression"}</div>
                <div style={{ fontSize:11, color:"var(--text3)", marginTop:1 }}>{lang === "en" ? "Generate a card for Instagram / WhatsApp" : "Générer une carte pour Instagram / WhatsApp"}</div>
              </div>
            </div>
        ...
        {weeklyChallenge && (
          <>
            <div style={{ fontFamily:"Outfit", fontSize:10, fontWeight:700, letterSpacing:1.2, textTransform:"uppercase", color:"var(--text3)", marginBottom:8 }}>
              {lang === "en" ? "Weekly challenge" : "Défi de la semaine"}
            </div>
            <div style={{ background: weeklyChallenge.done ? "var(--green-dim)" : "var(--gold-dim)", border:`1px solid ${weeklyChallenge.done ? "rgba(74,222,128,0.3)" : "rgba(201,168,76,0.3)"}`, borderRadius:16, padding:"14px 16px", marginBottom:16 }}>
              <div style={{ display:"flex", justifyContent:"space-between", alignItems:"flex-start", marginBottom:10 }}>
                <div>
                  <div style={{ fontFamily:"Outfit", fontSize:15, fontWeight:800, color: weeklyChallenge.done ? "var(--green)" : "var(--gold)" }}>
                    {weeklyChallenge.done ? (lang === "en" ? "🏆 Challenge completed!" : "🏆 Défi accompli !") : (lang === "en" ? "⚡ This week's target" : "⚡ Objectif cette semaine")}
                  </div>
                  <div style={{ fontSize:13, color:"var(--text)", marginTop:3, fontWeight:600 }}>{weeklyChallenge.name}</div>
                </div>
                <div style={{ textAlign:"right" }}>
                  {weeklyChallenge.target && (
                    <>
                      <div style={{ fontFamily:"Outfit", fontSize:20, fontWeight:800, color: weeklyChallenge.done ? "var(--green)" : "var(--gold)" }}>{weeklyChallenge.target}kg</div>
                      <div style={{ fontSize:10, color:"var(--text3)" }}>{lang === "en" ? `current PR: ${weeklyChallenge.current}kg` : `PR actuel: ${weeklyChallenge.current}kg`}</div>
                    </>
                  )}
                </div>
              </div>
              {/* Progress bar */}
              <div style={{ background:"var(--surface)", borderRadius:99, height:8, overflow:"hidden" }}>
                <div style={{ height:"100%", width:`${weeklyChallenge.progress}%`, background: weeklyChallenge.done ? "var(--green)" : "var(--gold)", borderRadius:99, transition:"width 0.6s ease" }}/>
              </div>
              <div style={{ fontSize:10, color:"var(--text3)", marginTop:5, fontFamily:"Outfit" }}>
                {weeklyChallenge.recentBest > 0
                  ? `${lang === "en" ? "Best this week" : "Meilleur cette semaine"}: ${weeklyChallenge.recentBest}kg — ${weeklyChallenge.progress}%`
                  : (lang === "en" ? "No log this week yet — go train!" : "Pas encore de log cette semaine — va t'entraîner !")}
              </div>
            </div>
          </>
        )}
        ...
        {exercises.length === 0 ? (
          <div style={{ textAlign:"center", padding:"48px 20px" }}>
            ...
            <div style={{ fontFamily:"Outfit", fontSize:16, fontWeight:700, color:"var(--text)", marginBottom:8 }}>
              {lang === "en" ? "No data yet" : "Aucune donnée"}
            </div>
            <div style={{ fontSize:13, color:"var(--text3)", marginBottom:20, lineHeight:1.6 }}>
              {lang === "en"
                ? "Complete workouts and log your sets to track your progress here."
                : "Effectue des séances et enregistre tes séries pour suivre ta progression ici."}
            </div>
            <button className="btn-primary" style={{ width:"100%", maxWidth:260, margin:"0 auto", display:"block" }} onClick={onGoCreate}>
              {lang === "en" ? "✦ Start a program" : "✦ Démarrer un programme"}
            </button>
          </div>
        ) : (
          <>
            ...
            {selEx && (
              <div style={{ background:"var(--surface)", border:"1px solid var(--border)", borderRadius:16, padding:"15px", marginBottom:14 }}>
                <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:10 }}>
                  <div>
                    <div style={{ fontFamily:"Outfit", fontSize:14, fontWeight:700, color:"var(--text)" }}>{selEx.name}</div>
                    <div style={{ fontSize:11, color:"var(--text3)", marginTop:2 }}>
                      {selEx.count} {lang === "en" ? "sets" : "séries"} · PR: <span style={{ color:"var(--gold)", fontWeight:700 }}>{selEx.pr > 0 ? `${selEx.pr}kg` : (lang === "en" ? "Bodyweight" : "Poids du corps")}</span>
                    </div>
                  </div>
                  <button onClick={() => setSelected(null)} style={{ background:"var(--surface2)", border:"1px solid var(--border)", borderRadius:8, padding:"5px 10px", fontSize:11, color:"var(--text3)", cursor:"pointer", fontFamily:"Outfit" }}>✕</button>
                </div>
                <SparkLine logs={selEx.logs} />
                <div style={{ marginTop:10, borderTop:"1px solid var(--border)", paddingTop:10 }}>
                  <div style={{ fontSize:10, fontWeight:700, letterSpacing:1, textTransform:"uppercase", color:"var(--text3)", marginBottom:7, fontFamily:"Outfit" }}>
                    {lang === "en" ? "Recent sets" : "Dernières séries"}
                  </div>
                  {[...selEx.logs].reverse().slice(0, 5).map((l, i) => {
                    const vol = Math.round((l.w || 0) * (l.r || 0));
                    return (
                      <div key={i} style={{ display:"flex", justifyContent:"space-between", alignItems:"center", padding:"6px 0", borderBottom: i < 4 ? "1px solid var(--border)" : "none" }}>
                        <div style={{ fontSize:11, color:"var(--text3)", fontFamily:"Outfit" }}>{l.d}</div>
                        <div style={{ display:"flex", gap:12, alignItems:"center" }}>
                          <div style={{ fontSize:13, fontWeight:700, color:"var(--text)", fontFamily:"Outfit" }}>{l.w > 0 ? `${l.w}kg` : "BW"}</div>
                          <div style={{ fontSize:12, color:"var(--text3)" }}>× {l.r}</div>
                          {vol > 0 && <div style={{ fontSize:11, background:"var(--surface2)", borderRadius:6, padding:"2px 7px", color:"var(--text2)", fontFamily:"Outfit" }}>{vol} vol</div>}
                        </div>
                      </div>
                    );
                  })}
            ...
            {/* All exercises list */}
            <div style={{ fontFamily:"Outfit", fontSize:10, fontWeight:700, letterSpacing:1.2, textTransform:"uppercase", color:"var(--text3)", marginBottom:8 }}>
              {lang === "en" ? "All exercises" : "Tous les exercices"}
            </div>
            {exercises.map(ex => {
              const trend = ex.logs.length >= 2 ? (ex.logs[ex.logs.length-1].w||0) - (ex.logs[0].w||0) : 0;
              const isSel = selected === ex.key;
              return (
                <div key={ex.key} onClick={() => setSelected(isSel ? null : ex.key)}
                  style={{ background: isSel ? "var(--gold-dim)" : "var(--surface)", border:`1px solid ${isSel ? "var(--gold)" : "var(--border)"}`, borderRadius:13, padding:"12px 14px", marginBottom:7, cursor:"pointer", WebkitTapHighlightColor:"transparent", transition:"all 0.15s" }}>
                  <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between" }}>
                    <div style={{ flex:1, minWidth:0 }}>
                      <div style={{ fontFamily:"Outfit", fontSize:13, fontWeight:700, color: isSel ? "var(--gold)" : "var(--text)", whiteSpace:"nowrap", overflow:"hidden", textOverflow:"ellipsis" }}>{ex.name}</div>
                      <div style={{ fontSize:10, color:"var(--text3)", marginTop:2 }}>
                        {ex.count} {lang === "en" ? "sets" : "séries"} · {lang === "en" ? "last" : "dernier"}: {ex.lastW > 0 ? `${ex.lastW}kg` : "BW"}
                      </div>
                    </div>
                    <div style={{ display:"flex", alignItems:"center", gap:7, flexShrink:0 }}>
                      {trend !== 0 && <div style={{ fontSize:11, fontWeight:700, color: trend > 0 ? "var(--green)" : "var(--red)", fontFamily:"Outfit" }}>{trend > 0 ? "▲" : "▼"}{Math.abs(trend)}kg</div>}
                      {ex.pr > 0 && <div style={{ background:"var(--gold-dim)", border:"1px solid rgba(201,168,76,0.25)", borderRadius:7, padding:"2px 8px", fontSize:10, fontWeight:700, color:"var(--gold)", fontFamily:"Outfit" }}>PR {ex.pr}kg</div>}
                      <div style={{ fontSize:10, color:"var(--text3)" }}>{isSel ? "▲" : "▼"}</div>
                    </div>
                  </div>
                </div>
              );
            ...
            {history.length > 0 && (
              <>
                <div style={{ fontFamily:"Outfit", fontSize:10, fontWeight:700, letterSpacing:1.2, textTransform:"uppercase", color:"var(--text3)", marginTop:20, marginBottom:8 }}>
                  {lang === "en" ? "Session history" : "Historique des séances"}
                </div>
                {history.slice(0, 10).map((h, i) => (
                  <div key={i} style={{ display:"flex", alignItems:"center", gap:10, background:"var(--surface)", border:"1px solid var(--border)", borderRadius:12, padding:"10px 13px", marginBottom:6 }}>
                    <div style={{ width:32, height:32, background:"var(--green-dim)", border:"1px solid rgba(74,222,128,0.2)", borderRadius:9, display:"flex", alignItems:"center", justifyContent:"center", fontSize:14, flexShrink:0, color:"var(--green)", fontWeight:700 }}>✓</div>
                    <div style={{ flex:1, minWidth:0 }}>
                      <div style={{ fontSize:12, fontWeight:600, color:"var(--text)", whiteSpace:"nowrap", overflow:"hidden", textOverflow:"ellipsis" }}>{h.dayName || h.programName}</div>
                      <div style={{ fontSize:10, color:"var(--text3)", marginTop:1 }}>{h.date}</div>
                    </div>
                    <div style={{ fontSize:10, color:"var(--green)", fontWeight:700, fontFamily:"Outfit" }}>DONE</div>
                  </div>
                ))}
              </>
            )}
...
// ── AUTH FORM (Supabase email login/signup) ───────────────────────────────────
function AuthForm({ lang }) {
  const [email, setEmail]       = useState("");
  const [password, setPassword] = useState("");
  ...
  const handle = async () => {
    ...
    else setMsg(mode === "signup"
      ? (lang === "en" ? "Check your email to confirm ✓" : "Vérifie ton email pour confirmer ✓")
      : (lang === "en" ? "Signed in — syncing…" : "Connecté — synchronisation…"));
  };
  ...
  return (
    <div style={{ display:"flex", flexDirection:"column", gap:8, marginTop:4 }}>
      ...
      <input type="password" placeholder={lang === "en" ? "Password (6+ chars)" : "Mot de passe (6+ car.)"} value={password} onChange={e => setPassword(e.target.value)}
        className="input-field" style={{ fontSize:14 }} />
      <button className="btn-primary" onClick={handle} disabled={loading} style={{ opacity: loading ? 0.7 : 1 }}>
        {loading ? "…" : mode === "login"
          ? (lang === "en" ? "Sign in" : "Se connecter")
          : (lang === "en" ? "Create account" : "Créer un compte")}
      </button>
      <button onClick={() => setMode(m => m === "login" ? "signup" : "login")}
        style={{ background:"none", border:"none", fontSize:12, color:"var(--text3)", cursor:"pointer", textAlign:"center", padding:4 }}>
        {mode === "login"
          ? (lang === "en" ? "No account? Sign up →" : "Pas de compte ? S'inscrire →")
          : (lang === "en" ? "Already have an account? Sign in" : "Déjà un compte ? Se connecter")}
      </button>
      {msg && <div style={{ fontSize:12, color:"var(--gold)", textAlign:"center", lineHeight:1.5 }}>{msg}</div>}
    </div>
  );
}

// ── PROFILE VIEW ──────────────────────────────────────────────────────────────
function ProfileView({ profile, history, onSave, lang, onLangChange, currentLang, theme = "dark", onThemeChange, sbUser, cloudSyncing }) {
  const t = (key) => TRANSLATIONS[lang]?.[key] ?? TRANSLATIONS.fr[key] ?? key;
        ...
        {activeSection === "settings" ? (
          <div className="fade-up">
            ...
            <div style={{ marginTop:18 }}>
              <div className="settings-item-label" style={{ marginBottom:6 }}>
                ☁️ {lang === "en" ? "Cloud sync" : "Synchronisation cloud"}
              </div>
              {!sb ? (
                <div style={{ background:"var(--surface2)", border:"1px solid var(--border)", borderRadius:12, padding:"11px 13px", fontSize:12, color:"var(--text3)", lineHeight:1.5 }}>
                  {lang === "en"
                    ? "Add your Supabase keys in App.jsx (SUPABASE_URL / SUPABASE_KEY) to enable multi-device sync."
                    : "Ajoute tes clés Supabase dans App.jsx (SUPABASE_URL / SUPABASE_KEY) pour activer la sync multi-appareils."}
                </div>
              ) : sbUser ? (
                <div>
                  <div style={{ background:"var(--green-dim)", border:"1px solid rgba(74,222,128,0.2)", borderRadius:12, padding:"11px 13px", marginBottom:8 }}>
                    <div style={{ fontSize:12, fontWeight:600, color:"var(--green)", marginBottom:2 }}>
                      {cloudSyncing ? "⟳ " : "✓ "}{lang === "en" ? "Synced" : "Synchronisé"}
                    </div>
                    <div style={{ fontSize:11, color:"var(--text3)" }}>{sbUser.email}</div>
                  </div>
                  <button
                    onClick={async () => { await sb.auth.signOut(); window.location.reload(); }}
                    style={{ background:"var(--surface2)", border:"1px solid var(--border)", borderRadius:10, padding:"9px 14px", fontSize:12, fontWeight:600, color:"var(--text2)", cursor:"pointer", fontFamily:"Outfit", width:"100%" }}>
