...
// ── CLOUD SYNC ────────────────────────────────────────────────────────────────
async function cloudPush(table, key, data) {
  if (!sb || !getSbUser()) { console.warn("[GAINZ] cloudPush skipped — no user"); return; }
  try {
    const { error } = await sb.from(table).upsert({
      user_id: getSbUser().id,
      key,
      data: JSON.stringify(data),
      updated_at: new Date().toISOString()
    }, { onConflict: "user_id,key" });
    if (error) console.warn("[GAINZ push error]", table, key, error.message);
  } catch(e) { console.warn("[GAINZ push exception]", e?.message); }
}
...
async function cloudPull() {
  ...
  try {
    ...
    const { data, error } = await sb.from("gainz_data").select("key,data").eq("user_id", uid);
    if (error) { console.warn("[GAINZ pull error]", error.message); return null; }
    const map = {};
    for (const row of data || []) {
      try { map[row.key] = JSON.parse(row.data); } catch { map[row.key] = row.data; }
    }
    return {
  ...
  } catch(e) { console.warn("[GAINZ pull exception]", e?.message); return null; }
}
...
async function savePrograms(list) {
  ...
  cloudPush("gainz_data", "programs", list);
}
...
async function saveProfile(p) {
  ...
  cloudPush("gainz_data", "profile", p);
}
...
async function saveHistory(h) {
  ...
  cloudPush("gainz_data", "history", h);
}
...
export default function App() {
  ...
  const pullAndMerge = async () => {
    ...
    // Snapshot local data before anything
    ...
    // Push local data up if cloud was empty for that key
    if (!cloud?.programs?.length && localPrograms.length) cloudPush("gainz_data", "programs", localPrograms);
    if (!cloud?.history?.length  && localHistory.length)  cloudPush("gainz_data", "history",  localHistory);
    if ((!cloud?.perf || !Object.keys(cloud.perf).length) && Object.keys(localPerf).length)  cloudPush("gainz_data", "perf",     localPerf);
    if (!cloud?.profile && localProfile)                  cloudPush("gainz_data", "profile",  localProfile);
