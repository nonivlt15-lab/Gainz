...
const CSS = `
  ...
  *,*::before,*::after{box-sizing:border-box;margin:0;padding:0}
  ...
  .fullbody-card{
    ...
    transition:all 0.15s;-webkit-tap-highlight-color:transparent;position:relative;
    border-left:3px solid var(--border);
  }
  .fullbody-card::before{content:'';position:absolute;left:0;top:0;bottom:0;width:3px;background:var(--border);border-radius:0 2px 2px 0;transition:background 0.2s}
  ...
  .fullbody-icon{font-size:18px;color:var(--text3);font-family:'Outfit',sans-serif;font-weight:800;width:36px;height:36px;background:var(--surface2);border:1px solid var(--border);border-radius:10px;display:flex;align-items:center;justify-content:center}
  .fullbody-card.selected .fullbody-icon{background:var(--gold-dim);border-color:rgba(201,168,76,0.3);color:var(--gold)}
  .fullbody-name{fontFamily:'Outfit',sans-serif;font-size:14px;font-weight:700;color:var(--text)}
  .fullbody-card.selected .fullbody-name{color:var(--gold)}
  .fullbody-sub{font-size:11px;color:var(--text3);margin-top:2px}
  .fullbody-check{font-size:14px;color:var(--gold);font-weight:700;font-family:'Outfit',sans-serif}
  .adv-toggle{
              ...
              return (
                <div key={si} style={{ marginBottom: 10 }}>
                  <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between", marginBottom:6 }}>
                    <div>
                      <div style={{ fontFamily:"Outfit", fontSize:11, fontWeight:700, letterSpacing:1, textTransform:"uppercase", color:"var(--text3)" }}>{section.group}</div>
                      <div style={{ fontSize:10, color:"var(--text3)", opacity:0.7, marginTop:1, fontStyle:"italic" }}>{section.hint}</div>
                    </div>
                    <button
                      onClick={() => {
                      ...
                      style={{ fontSize:10, fontWeight:600, color: allSelected ? "var(--gold)" : "var(--text3)", background:"none", border:"none", cursor:"pointer", fontFamily:"Outfit", padding:"4px 0", letterSpacing:0.3 }}>
                      {allSelected ? (lang === "en" ? "Deselect all" : "Tout désélect.") : (lang === "en" ? "Select all" : "Tout sélect.")}
                    </button>
                  </div>
